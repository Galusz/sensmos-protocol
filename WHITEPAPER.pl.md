...
### Żywa mapa
Każdy publiczny odczyt zasila żywą mapę, feed zdarzeń i heatmapę wartości. Każdy node ma obszar **pokrycia**, którego promień dostosowuje się do lokalnej gęstości i do globalnej skali sieci: samotny node w nieobsłużonym regionie pokrywa szeroki obszar; gęste miejskie skupisko kurczy się do ciasnych, nienakładających się kółek.
...
- **Geolokalizacja app-proof.** Pozycja noda jest ustawiana wyłącznie z GPS telefonu w apce (kotwica dowodu), opcjonalnie rozmywana 200–800 m dla prywatności; czytelne miasto liczy serwer. Pozycji nie da się wpisać ręcznie — to wiąże deklarację lokalizacji z fizycznym telefonem obecnym przy nodzie.
...
Razem czyni to fałszowanie kosztownym: żeby udawać nagrody, potrzebujesz realnego sprzętu, realnego klucza, realnego telefonu w realnej lokalizacji, przechodzącego challenge w czasie rzeczywistym.
...
```
waga       = scarcity × kategorie × uptime × aktywność        (max ≈ 2.14×)
...
1. **Live (teraz).** Kontrakt wdrożony na Polygonie; dobowe epoki mintują i publikują cumulative rooty; firmware, apka, integracja HA i dokumentacja protokołu open-source.
...
3. **Utwardzenie zaufania.** Owner → multisig + timelock, audyt zewnętrzny, weryfikacja na Polygonscanie, publiczne zbiory danych nagród + monitoring.
4. **Weryfikowalność.** Przesunięcie oracle nagród z „zaufanego" ku „challengeable / dowodliwemu", gdy wartość to uzasadni.
